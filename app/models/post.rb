class Post < ActiveRecord::Base
  ...
  validates_format_of :url, with: %r{https?:\/\/[\S]+}, message: 'URL field must be a valid URL (ex. http://www.example.com)'
  ...
  def hide
    Post.transaction do
      ...
      self.comments.each do |comment|
        comment.votes.each(&:destroy)
        comment.flags.each(&:destroy)
        comment.update(hidden: true)
      end
      self.categories.each(&:reduce_unhidden_posts_count)
    end
  end

  def increase_unhidden_comments_count
